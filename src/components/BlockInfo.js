...
import { CRYPTIX_UNIT } from "../explorer_constants.js";
import {numberWithCommas} from "../helper.js";
import {getBlock, getTransactions} from '../cryptix-api-client.js';
                                        ...
                                        <Row className="utxo-border py-3">
                                                                    ...
                                                                    <Col xs={12} sm={8} md={9} lg={9} xl={8} xxl={7}
                                                                        ...
                                                                        <CopyButton
                                                                            text={getAddrFromOutputs(txInfo[txInput.previousOutpoint.transactionId]["outputs"], txInput.previousOutpoint.index || 0)}/>
                                                                               ...
                                                                               sm={4} md={2}>
                                                                    -{numberWithCommas(getAmountFromOutputs(txInfo[txInput.previousOutpoint.transactionId]["outputs"], txInput.previousOutpoint.index || 0))}&nbsp;{CRYPTIX_UNIT}
                                                                </Col></>
                                                                :
                                                                <><Col xs={12} sm={8} md={9} lg={9} xl={8} xxl={7}
                                                                       className="text-truncate">
                                                                    <a className="blockinfo-link"
                                                                       href={`https://katnip.cryptixd.net/tx/${txInput.previousOutpoint.transactionId}`}
                                                                       target="_blank">
                                                                        TX
                                                            ...
                                                            <Col xs={12} sm={8} md={9} lg={9} xl={8} xxl={7}
                                                                ...
                                                                <CopyButton
                                                                    text={txOutput.verboseData.scriptPublicKeyAddress}/>
                                                                       ...
                                                                       md={3}>+{numberWithCommas(txOutput.amount / 100000000)}&nbsp;{CRYPTIX_UNIT}</Col>
                                                        </Row>)}
                                                    </Container>
                                                </Col>
                                            </Col>
                                            <Col sm={5} md={4} lg={3}>
                                                ...
                                                <div className="utxo-value d-flex flex-row">
                                                    <div
                                                        className="utxo-amount">{(numberWithCommas(tx.outputs.reduce((a, b) => (a || 0) + parseInt(b.amount), 0) / 100000000))} {CRYPTIX_UNIT}
                                                    </div>
                                                </div>
                                            </Col>
                                            {/*<Col sm={3} md={2}>*/}
                                            ...
                                            {/*        {tx.inputs.length > 0 ?*/}
                                            {/*            <div*/}
                                            {/*                className="">{!!txInfo && tx.inputs.length === txInfo.length && sompiOrCytx((tx.inputs.reduce((a, c) => a + getAmountFromOutputs(txInfo[c.previousOutpoint.transactionId]?.outputs, c.previousOutpoint.index), 0)) - (tx.outputs.reduce((a, b) => (a || 0) + parseInt(b.amount), 0) / 100000000))}*/}
                                            {/*            </div> : "0"}*/}
                                            {/*    </div>*/}
                                            {/*</Col>*/}
                                            <Col sm={3} md={2}>
